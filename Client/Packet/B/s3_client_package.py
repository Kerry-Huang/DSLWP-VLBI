...
cp.read('./Client/Packet/B/config.ini')
######################################
...
class S3_percentage(object):
	...
	def __call__(self, bytes_amount):
		...
		print(
			"\033[22;0H%15s %15d %15d  (%.2f%%)" 
			% (self.timestamp, self.seen_so_far, self.size, percentage)
		)
...
class cutter:
	...
	def get_timestamp(self):
		...
		try:
			...
			self.new_timestamp = int(math.floor(float(self.timeapi.read())-18))
		except Exception as e:
